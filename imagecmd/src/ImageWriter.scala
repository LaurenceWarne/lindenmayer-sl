...
object ImageWriter {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val cellsToColour: Set[(Int, Int)] = interpreter.interpret(
      shape, dragonCurveTranslator
    ).toSet
    val img = getCenteredImage(cellsToColour, width, height)
    writeImage(img, args.head)
  }

  def getCenteredImage(cellsToColour: Iterable[(Int, Int)], width: Int, height: Int): BufferedImage = {
